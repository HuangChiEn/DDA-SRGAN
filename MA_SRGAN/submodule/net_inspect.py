...
if __name__ == "__main__":
    ...
    wei_path = "../../pretrain/MA_SRGAN/CelebA/generator/000.h5"
    ...
    sub_mod = model
    # prepare the image (e.g. scale pixel values for the vgg)
    im_paths = glob.glob("./masr_sample/*")
    ...
    for img in img_lst:
        fea_maps = sub_mod.predict(img)
        fea_maps = fea_maps * 0.5 + 0.5
        feature_map = fea_maps.sum(axis=3)
    ...
    for fea_map in feature_map_lst:
        fig = plt.imshow(fea_map, cmap=plt.cm.hot_r) # plt.cm.hot_r
